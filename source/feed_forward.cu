...
__global__ void eval_layer(int num_threads, int layer, int num_weights, int num_neurons, float *input, float *weights, float *outputs)
{
    ...
    __shared__ float output_reduce[MAX_NUM_NEURONS]; 
    ...
    if(neuron_index < num_neurons && weight_index < num_weights){
        if(layer == 0){//first layer so read from the input data
            ...
            output_reduce[weight_index] = weights[neuron_weight_index]*outputs[(layer)*MAX_NUM_NEURONS+weight_index];
            if(weight_index2 && weight_index2 < num_weights){
                output_reduce[weight_index2] = weights[neuron_weight_index2]*outputs[(layer)*MAX_NUM_NEURONS+weight_index2];
            }
        }
        ...
        if(weight_index == 0){
            ...
            if(outputs[layer*MAX_NUM_NEURONS + neuron_index] <= -1.0){
                outputs[layer*MAX_NUM_NEURONS + neuron_index] = (float)-0.8;
            }
            else if(outputs[layer*MAX_NUM_NEURONS + neuron_index] >= 1.0){
                outputs[layer*MAX_NUM_NEURONS + neuron_index] = (float)0.8;
            }
        }
        __syncthreads();
...
eval_network(int num_threads, int num_layers, int num_weights, int num_neurons, float *input, float *weights, 
    ...
    if(neuron_index < num_neurons && weight_index < num_weights){
        ...
        layer_input[neuron_index] = input[neuron_index];
        if(weight_index2 && weight_index2 < num_weights){
    ...
    for(int layer = 0; layer < num_layers-1; layer++){
        if(neuron_index < num_neurons && weight_index < num_weights){
            output_reduce[weight_index] = weights[num_weights*num_neurons*layer + num_weights*neuron_index + weight_index]*layer_input[weight_index];
            if(weight_index2 && weight_index2 < num_weights){
                output_reduce[weight_index2] = weights[num_weights*num_neurons*layer + num_weights*neuron_index + weight_index2]*layer_input[weight_index2];
            }
        }
        //calculated all the weights for each neuron, so now sync
        __syncthreads();
        ...
        if(weight_index == 0){
            ...
            if(layer_input[neuron_index] <= -1.0)
                layer_input[neuron_index] = (float)-0.8;
            else if(layer_input[neuron_index] >= 1.0)
                layer_input[neuron_index] = (float)0.8;
    ...
    if(neuron_index < NUM_OUTPUT_NEURONS){
        output_reduce[weight_index] = weights[num_weights*num_neurons*(num_layers-1) + num_weights*neuron_index + weight_index]*layer_input[weight_index];
            if(weight_index2 && weight_index2 < num_weights){
                output_reduce[weight_index2] = weights[num_weights*num_neurons*(num_layers-1) + num_weights*neuron_index + weight_index2]*layer_input[weight_index2];
            }
        }
        //calculated all the weights for each neuron, so now sync
        __syncthreads();
        ...
        if(weight_index == 0){
            ...
            if(outputs[(num_layers-1)*MAX_NUM_NEURONS + neuron_index] <= -1.0)
                outputs[(num_layers-1)*MAX_NUM_NEURONS + neuron_index] = (float)-0.8;
            else if(outputs[(num_layers-1)*MAX_NUM_NEURONS + neuron_index] >= 1.0)
                outputs[(num_layers-1)*MAX_NUM_NEURONS + neuron_index] = (float)0.8;
        }
}
